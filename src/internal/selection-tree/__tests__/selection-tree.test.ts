...
import { SelectionTree, GroupSelectionState } from '../index';
...
const createSelectionTree = (
  ...
  const { roots, getChildren } = buildTree(allItems, getId, getParentId);
  return new SelectionTree(roots, { getChildren, trackBy, isComplete }, state);
};

function buildTree<T>(allItems: readonly T[], getId: (item: T) => string, getParentId: (item: T) => string | null) {
  ...
  for (const item of allItems) {
    if (!getParentId(item)) {
      ...
      const bucket = childrenById.get(getParentId(item)!) ?? [];
      bucket.push(item);
      childrenById.set(getParentId(item)!, bucket);
    }
  }
  return { roots, getChildren: (item: T): T[] => childrenById.get(getId(item)) ?? [] };
}
...
test('tracks items by reference', () => {
  ...
  const { roots, getChildren } = buildTree(allItems, getId, getParentId);
  const tree = new SelectionTree(roots, { getChildren }, { inverted: true, toggledItems: [roots[0], { id: 'b' }] });
...
test('tracks items with trackBy', () => {
  ...
  const tree = new SelectionTree(
    ...
    { inverted: true, toggledItems: [roots[0], { id: 'b' }] }
  );
