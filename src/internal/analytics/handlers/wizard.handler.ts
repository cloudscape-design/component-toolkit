...
import { getFunnelSubsteps } from '../utils/browser';
import { getFunnel } from '../utils/funnel';
import { getLastBreadcrumbText } from '../utils/page';

export const mount: Handler<WizardMountDetail> = event => {
  ...
  const funnelConfig: FunnelConfig = {
    name: funnelName,
    type: 'multi-page',
    stepConfiguration: event.detail?.config.stepConfiguration || [],
  };
  ...
  // FIXME: Fix typescript types here
  const funnelSteps = event.detail!.config.stepConfiguration.map(stepConfig => new FunnelStep(stepConfig));
  const activeStep =
    funnelSteps[event.detail!.config.activeStepIndex] ||
