...
export class FunnelSubstep {
  ...
  start() {
    ...
    console.log('Funnel substep started', { name: this.name, number: this.number });
  ...
  complete() {
    ...
    console.log('Funnel substep completed', { name: this.name, number: this.number });
  ...
  setFieldError(fieldId: string, fieldErrorId: string) {
    if (!fieldErrorId) {
      ...
      console.log('Funnel substep error cleared', { name: this.name, number: this.number, fieldId });
      return;
    }

    this.errors.set(fieldId, fieldErrorId);
    this.state = 'error';
    console.log('Funnel substep error', { name: this.name, number: this.number, fieldId, fieldErrorId });
  }
}
...
export class FunnelStep {
  ...
  start() {
    console.log('Funnel step started', { name: this.name, number: this.number });
  }

  complete() {
    this.activeSubstep?.complete();
    console.log('Funnel step completed', { name: this.name, number: this.number });
  }

  error() {
    this.state = 'error';
    console.log('Funnel step errored', { name: this.name, number: this.number });
  }
...
export class Funnel {
  ...
  start() {
    ...
    console.log('Funnel started', { name: this.name, type: this.type });
    this.state = 'in-progress';
  ...
  complete() {
    ...
    this.state = 'completed';
    console.log('Funnel completed', { name: this.name, type: this.type, result });
  }

  cancel() {
    this.state = 'cancelled';
    console.log('Funnel cancelled', { name: this.name, type: this.type });
  }

  submit() {
    this.state = 'submitting';
    console.log('Funnel submitting', { name: this.name, type: this.type });
  }

  error() {
    this.state = 'error';
    console.log('Funnel errored', { name: this.name, type: this.type });
  }
  ...
  interaction(type: 'info' | 'external') {
    ...
    switch (type) {
      case 'info':
        console.log('Funnel info link interaction', props);
        break;
      case 'external':
        console.log('Funnel external link interaction', props);
        break;
    }
  }
